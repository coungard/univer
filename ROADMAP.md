...
| Задача | Модуль | Результат |
|---|---|---|
| Подгруппы (`п/гр`) | `Student`/`Pair` (или новая сущность `Subgroup`) | Лабораторные/практики, идущие по подгруппам с чередованием по чётности недели, отражаются без огрубления до целой группы |
| Тип занятия и форма контроля | `Course`/`Pair` | `лк`/`пз`/`лб`, `зач`/`экз` не теряются при импорте — по аналогии с `room`, добавленным в Этапе 4 |
| Реальное звонковое расписание (время по номеру пары) | справочник/конфиг | `Pair.startTime`/`endTime` заполняются по факту, а не по иллюстративному предположению из Сценария 3 |
| Семантика «1-3 недели» vs `WeekParity` | `WeekParity`/`Pair` | Понятно, что означает диапазон недель в реальных расписаниях — `ODD` целиком или фиксированный блок; при необходимости модель расширяется |
| «Слэш»-ячейки (параллельные подгруппы в одном слоте) | импорт расписания | Каждая подгруппа/поток — отдельный `Pair`, а не текст, слитый в одно поле |
| Предмет → кафедра (`department_id`) | `Course` | Однозначное соответствие для всех предметов реального расписания, включая спорные (Математика, Программирование, БЖД, Правов. ОПД) |
| Миграция с реальным расписанием 1 курса КТиЭ | `data/V13+` | Расписание из `schedules/ktie_kurs1_nedeli1-3.txt` целиком в БД |
...
**Следующий шаг → Этап 6 (Нормализация данных для расписания):** подгруппы, тип занятия/форма контроля,
звонковое расписание, семантика «1-3 недели», сопоставление предмет → кафедра — на реальных данных из
`schedules/ktie_kurs1_nedeli1-3.txt`. Этап 7 (Прод-готовность: секреты в env, Dockerfile приложения, явная настройка CORS,
Spring-профили, CI/CD) — после него.
