...
**Статус на 18.08.2026:** Этапы 1–4 закрыты. Этап 4 «Лекции» реализован целиком: `Lecture` (#30) — CRUD,
генерация из шаблона `Pair` (валидация дня недели и чётности от `Semester.startDate`, запрет повторной
генерации), `Lecture`↔`Group` многие-ко-многим (поток); права доступа (#31) — `ADMIN` и `TEACHER` оба
могут редактировать/удалять любой `Course`/`Lecture`, без ограничения по своей кафедре/факультету
(осознанный выбор — простая модель вместо резолвинга владения). Заодно исправлен тот же класс бага, что
и `courses.teacher_id` (V7), теперь для `lectures.teacher_id` (V9). Полный `mvn test` — 132/132 зелёных.

**Следующий шаг → Этап 5 (Зачисление и посещаемость):** `Enrollment` → `LectureAttendance`.
