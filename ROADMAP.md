...
**Статус на 18.08.2026:** Этапы 1–3 закрыты, Этап 4 «Лекции» частично закрыт — `Lecture` (#30) готова:
CRUD, генерация из шаблона `Pair` (валидация дня недели и чётности от `Semester.startDate`, запрет
повторной генерации), `Lecture`↔`Group` многие-ко-многим (поток). Заодно исправлен тот же класс бага, что
и `courses.teacher_id` (V7), теперь для `lectures.teacher_id` (V9). Полный `mvn test` — 132/132 зелёных.
Открыты #31 (права доступа на курсы/лекции) и #32 (тесты Lecture — фактически уже написаны вместе с #30,
issue стоит закрыть).

**Следующий шаг → Этап 4:** issue #31 (модель прав доступа для `Course`/`Lecture`), затем Этап 5
