...
- **Целевая модель расписания зафиксирована в `TARGET.md`** — циклическое расписание по чётности недели
  (`StudyYear` → `Semester` → `Group` → `WeekScheduleCycle`/`Pair`) нигде в коде ещё не спроектировано.
  Roadmap ниже перестроен так, чтобы `Lecture` проектировалась сразу с оглядкой на неё, а не переделывалась
  повторно после появления `Group`/`Semester`.
...
**Статус на 18.08.2026:** Этап 1 и Этап 2 закрыты, issue #29 закрыт на GitHub. Этап 2 («Курсы и лекции»)
переименован в «Учебные курсы» и сокращён до `Course` (✅ issue #29, закрыт) — `Lecture` и решение по правам
доступа (issue #31) перенесены в новый Этап 4, который теперь стоит после Этапа 3 «Циклическое расписание»
(`StudyYear`/`Semester`/`Group`/`WeekScheduleCycle`), зафиксированного в `TARGET.md`. Этап «Зачисление и
посещаемость» сдвинут на 5, «Прод-готовность» — на 6.

**Следующий шаг → Этап 3 (Циклическое расписание)**
