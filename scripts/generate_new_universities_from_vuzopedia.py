...
# помечаем как "требует уточнения". Аналогично "Приемная комиссия ... г. X" -- это не
# отдельный вуз, а региональная приёмная комиссия одного и того же вуза (АНО ВО ММУИТ
# "Академия ТОП"), продублированная на сотню городов.
SUB_ENTITY_RE = re.compile(r"факультет|кафедра|приемная комиссия|приёмная комиссия", re.IGNORECASE)
